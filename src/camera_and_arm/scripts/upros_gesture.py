...
class UP_Gesture:
    ...
    def detectNumber(self, hand_landmarks, img):
        ...
        # 提取 y 坐标
        thumb_tip_y = hand_landmark[thumb_tip_id].y * h      # 拇指指尖的 Y 坐标
        index_tip_y = hand_landmark[index_finger_tip_id].y * h   # 食指指尖的 Y 坐标
        middle_tip_y = hand_landmark[middle_finger_tip_id].y * h # 中指指尖的 Y 坐标
        ring_tip_y = hand_landmark[ring_finger_tip_id].y * h     # 无名指指尖的 Y 坐标
        pinky_tip_y = hand_landmark[pinky_finger_tip_id].y * h   # 小指指尖的 Y 坐标
        pinky_mcp_y = hand_landmark[pinky_finger_mcp_id].y * h   # 小指根部的 Y 坐标
        wrist_y = hand_landmark[wrist_id].y * h                  # 手腕的 Y 坐标

        # 提取 x 坐标
        thumb_tip_x = hand_landmark[thumb_tip_id].x * w          # 拇指指尖的 X 坐标
        index_tip_x = hand_landmark[index_finger_tip_id].x * w   # 食指指尖的 X 坐标
        middle_tip_x = hand_landmark[middle_finger_tip_id].x * w # 中指指尖的 X 坐标
        ring_tip_x = hand_landmark[ring_finger_tip_id].x * w     # 无名指指尖的 X 坐标
        pinky_tip_x = hand_landmark[pinky_finger_tip_id].x * w   # 小指指尖的 X 坐标
        pinky_mcp_x = hand_landmark[pinky_finger_mcp_id].x * w   # 小指指尖的 X 坐标
        wrist_x = hand_landmark[wrist_id].x * w                  # 手腕的 X 坐标
        ...
        if (dist_index2wrist_ratio < 1.9 and dist_middle2wrist_ratio < 1.8 and
            dist_ring2wrist_ratio < 1.6 and dist_pinky2wrist_ratio < 1.4 and
            dist_pinky_mcp2wrist_ratio < 0.8):
            return 0

        # 食指伸直，距离手腕较远（>2.0），其余手指弯曲，小指掌关节距离手腕近，食指伸直，返回 1
        elif (2.0 < dist_index2wrist_ratio and dist_middle2wrist_ratio < 1.8 and
              dist_ring2wrist_ratio < 1.6 and dist_pinky2wrist_ratio < 1.4 and
              dist_pinky_mcp2wrist_ratio < 0.8):
            return 1

        # 食指、中指伸直，距离手腕远（>2.0和>1.9），无名指和小指弯曲，返回 2
        elif (2.0 < dist_index2wrist_ratio and 1.9 < dist_middle2wrist_ratio and
              dist_ring2wrist_ratio < 1.6 and dist_pinky2wrist_ratio < 1.4 and
              dist_pinky_mcp2wrist_ratio < 0.8):
            return 2
        ...
        # 食指、中指、无名指伸直，距离手腕远（>2.0/1.9/1.75），小指弯曲，返回 3
              ...
              1.75 < dist_ring2wrist_ratio and dist_pinky2wrist_ratio < 1.4 and
              dist_pinky_mcp2wrist_ratio < 0.8):
            return 3

        # 食指至小指伸直，距离手腕均较远（>2.0/1.9/1.75/1.5），小指掌关节距离近，返回 4
        elif (2.0 < dist_index2wrist_ratio and 1.9 < dist_middle2wrist_ratio and
              1.75 < dist_ring2wrist_ratio and 1.5 < dist_pinky2wrist_ratio and
              dist_pinky_mcp2wrist_ratio < 0.8):
            return 4

        # 所有指尖距离手腕较远（>0.5），小指掌关节与手腕距离适中（0.9~1.2），返回 5
        elif (dist_index2wrist_ratio > 0.5 and dist_middle2wrist_ratio > 0.5 and
              dist_ring2wrist_ratio > 0.5 and 0.9 < dist_pinky_mcp2wrist_ratio < 1.2):
            return 5
